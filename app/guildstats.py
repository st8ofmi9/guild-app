		...
		for obj in withdrawal:
			...
			try:
			  self.item = self.guild.apicall(self.itemcall.format(withdrawal[self.i]['item_id']),1)
				...
				print("Hmmm, Something went wrong with {}. Logging results".format(withdrawal[self.i]['item_id']))
				pass
...
if __name__ == '__main__':
	...
	f = open('/opt/dw/staging/www/hw.html','w')
	for rank in ranks: 
  ...
  {1!s}
	...
	</html>""".format(lst2string(Firey), lst2string(Bomb))
	f.write(message)
	f.close()	